...
import pytest
    ...
    def create_stats_bill
    bill_manager = BillManager()
    bills = bill_manager.create_bills_000001()
    return Statistics(bills)

    
    def create_stats_bill
    bill_manager = BillManager()
    bills = bill_manager.create_bills_000002()
    return Statistics(bills)   

    
    def create_stats_bill
    bill_manager = BillManager()
    bills = bill_manager.create_bills_000003()
    return Statistics(bills) 

    
    def create_stats_bill
    bill_manager = BillManager()
    bills = bill_manager.create_bills_000004()
    return Statistics(bills) 

    
    def test_find_top_sell_product_000001():
    top_prodict = create_stats_bills_000001().find_top_sell_product()
    assert top_product[0].product_id == "X10001", "Check the function: find_top_sell_product"
    assert top_product[1] == 1, "Check the function: find_top_sell_product"

    
    def test_find_top_sell_product_000002():
    top_prodict = create_stats_bills_000002().find_top_sell_product()
    assert top_product[0].product_id == "X10002", "Check the function: find_top_sell_product"
    assert top_product[1] == 1, "Check the function: find_top_sell_product"

    
    def test_find_top_sell_product_000003():
    top_prodict = create_stats_bills_000003().find_top_sell_product()
    assert top_product[0].product_id == "X10003", "Check the function: find_top_sell_product"
    assert top_product[1] == 1, "Check the function: find_top_sell_product"

    
    def test_find_top_sell_product_000004():
    top_prodict = create_stats_bills_000004().find_top_sell_product()
    assert top_product[0].product_id == "X10004", "Check the function: find_top_sell_product"
    assert top_product[1] == 1, "Check the function: find_top_sell_product"


    def test_find_top_two_sellers_000001():
    list_sellers = create_stats_bills_000001().find_top_two_sellers()
    assert list_sellers[0].dni == "X101", "Check the function: find_top_two_sellers"
    assert len(list_sellers) == 1, "Check the function: find_top_two_sellers"

    
    def test_find_top_two_sellers_000002():
    list_sellers = create_stats_bills_000002().find_top_two_sellers()
    assert list_sellers[0].dni == "X101", "Check the function: find_top_two_sellers"
    assert len(list_sellers) == 1, "Check the function: find_top_two_sellers"
 
    
    def test_find_top_two_sellers_000003():
    list_sellers = create_stats_bills_000003().find_top_two_sellers()
    assert list_sellers[0].dni == "X101", "Check the function: find_top_two_sellers"
    assert len(list_sellers) == 1, "Check the function: find_top_two_sellers"

    
    def test_find_top_two_sellers_000004():
    list_sellers = create_stats_bills_000004().find_top_two_sellers()
    assert list_sellers[0].dni == "X102", "Check the function: find_top_two_sellers"
    assert list_sellers[1].dni == "X104", "Check the function: find_top_two_sellers"
    assert len(list_sellers) == 2, "Check the function: find_top_two_sellers"
    
     
    def test_find_buyer_lowest_total_purchases_000001():
    tuple_buyer = create_stats_bills_000001().find_buyer_lowest_total_purchases()
    assert tuple_buyer[0].dni == "X108", "Check the function: find_buyer_lowest_total_purchases"
    assert tuple_buyer[1] == 180.2, "CCheck the function: find_buyer_lowest_total_purchases"
   
    
    def test_find_buyer_lowest_total_purchases_000002():
    tuple_buyer = create_stats_bills_000002().find_buyer_lowest_total_purchases()
    assert tuple_buyer[0].dni == "X108", "Check the function: find_buyer_lowest_total_purchases"
    assert tuple_buyer[1] == 360.60, "Check the function: find_buyer_lowest_total_purchases"def test_find_buyer_lowest_total_purchases_EX101():
    
   
    def test_find_buyer_lowest_total_purchases_000003():
    tuple_buyer = create_stats_bills_000003().find_buyer_lowest_total_purchases()
    assert tuple_buyer[0].dni == "X109", "Check the function: find_buyer_lowest_total_purchases"
    assert tuple_buyer[1] == 170.76, "CCheck the function: find_buyer_lowest_total_purchases"def test_find_buyer_lowest_total_purchases_EX101():

    
    def test_find_buyer_lowest_total_purchases_000004():
    tuple_buyer = create_stats_bills_000004().find_buyer_lowest_total_purchases()
    assert tuple_buyer[0].dni == "X108", "Check the function: find_buyer_lowest_total_purchases"
    assert tuple_buyer[1] == 75, "CCheck the function: find_buyer_lowest_total_purchases"

    
    def test_order_products_by_tax_000001():
    list_products = create_stats_bill_000001(
    ).order_products_by_tax(order_type=OrderType.ASC)
    assert list_products[0][0].product_id == ""X10001", "Check the function: order_product_by_tax"
    assert len(list_products) == 1, "Check the function: order_product_by_tax"

        
    def test_order_products_by_tax_000002():
    list_products = create_stats_bill_000002(
    ).order_products_by_tax(order_type=OrderType.ASC)
    assert list_products[0][0].product_id == "X10002", "Check the function: order_product_by_tax"
    assert len(list_products) == 2, "Check the function: order_product_by_tax"

   
    def test_order_products_by_tax_000003():
    list_products = create_stats_bills_000003(
    ).order_products_by_tax(order_type=OrderType.ASC)
    assert list_products[0][0].product_id == "X10002", "Check the function: order_products_by_tax"
    assert len(list_products) == 2, "Check the function: order_products_by_tax"


    def test_order_products_by_tax_000004():
    list_products = create_stats_bills_000004(
    ).order_products_by_tax(order_type=OrderType.ASC)
    assert list_products[0][0].product_id == "000002", "Check the function: order_products_by_tax"
    assert list_products[1][0].product_id == "000003", "Check the function: order_products_by_tax"
    assert list_products[2][0].product_id == "000004", "Check the function: order_products_by_tax"
